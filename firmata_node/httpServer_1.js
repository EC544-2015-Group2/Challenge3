                    ...
                    if (deviceID.filter(function(element) {
                        ...
                        if (url[2] === 'pin') {
                            if (pinID.filter(function(element) {
                                ...
                                var xbeeStream = XbeeApiStream(url[1], Serial, xbeeAPI);
                        		var arduino = new firmata.Board(xbeeStream, function() {
                                	console.log('Accessed board pins, attempting to write to specified pin.');
                                }).length > 0) {
                                if (url[5] === 'HIGH') {
                                	...
                                	response.end('Error: Please specify HIGH or LOW')
                                }
                            } else {
                                response.end('Error: url should be of format /device/deviceID/pin/pinID/HIGH or LOW') // send error in request
                            }
                        } else {
                            response.end('Error: url should be of format /device/deviceID/pin/pinID/HIGH or LOW') // send error in request
                        }
